...
class Unicalc
{
 ...
 public AST S()
 {
  ...
  if(!isNumber(next) && !isAlphabetic(next)){
    throw new ParseError("ParseError: Expected number or identifier or subexpression, but found: 'null'");
  }
 ...
 public AST Q()
 {
  ...
  if(isAlphabetic(next) || isNumber(next) || "(".equals(next)){
  // toks.pop();
 ...
 public AST R()
 {
  ...
  if(next.equals("^") ){
   ...
   return new Power(leftTree,value);
  }
  else{
    return leftTree;                       
  }
