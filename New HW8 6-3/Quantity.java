...
public class Quantity{
  ...
  public Quantity pow(int powerBy)
  {
    ...
    Set<Map.Entry<String,Integer>> set = this.unit.entrySet();
    for(Entry<String,Integer> entry : set)
    {
      powQuantity.unit.put(entry.getKey(), entry.getValue() * powerBy);
    }
    return powQuantity;
  }
  ...
  {
    Quantity retQuantity = new Quantity(1, Arrays.asList(unitName), Arrays.asList(""));
       //if key (unit name) isn't located in the database
    if(!database.containsKey(unitName)){
      System.out.println("in");
      retQuantity = new Quantity(1, Arrays.asList(unitName), Arrays.asList(""));
      return retQuantity;
    }
    else{
  ...
  public Quantity normalize(Map<String,Quantity> database)
  {
    ...
    for(Entry<String,Integer> entry : set)
    {
      if(!database.containsKey(entry.getKey())){
        System.out.println("in");
        newQuan  = new Quantity(this.value, Arrays.asList(entry.getKey()), Arrays.asList(""));
        return newQuan;
      }
  ...
  {
    Quantity quan = new Quantity(20, Arrays.asList("liter"), Arrays.asList(""));
    Quantity quan2 = new Quantity(9.0, Arrays.asList("meter"),Collections.<String>emptyList());
    System.out.println(quan.normalize(QuantityDB.getDB()));
        System.out.println(quan2.normalize(QuantityDB.getDB()));
  }
