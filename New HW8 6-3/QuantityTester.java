...
public class QuantityTester extends TestCase {
 ...
 public void testNormalize()
 {
  ...
  Quantity qNormalize2 = new Quantity(9.0, Arrays.asList("meter"),Collections.<String>emptyList());
  assertEquals(new Quantity(1,Collections.<String>emptyList(), Collections.<String>emptyList()).toString(), qNormalize.normalize(db).toString());
  assertEquals(new Quantity(9.0,Arrays.asList("meter"), Collections.<String>emptyList()).toString(), qNormalize2.normalize(db).toString());
 }
 
 public void testPow()
 {
  Quantity qPow = new Quantity(4, Arrays.asList("m"), Arrays.asList("s"));
  assertEquals(new Quantity(16.0, Arrays.asList("m", "m"), Arrays.asList("s", "s")).toString(), qPow.pow(2).toString());
 }
